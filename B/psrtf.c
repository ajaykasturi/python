...
void sort(int at[],int bt[],int pn[],int n,int btt[])
{
	int i,j;
	for(i=0;i<n-1;i++)
	{
...
int findMinJob(int at[],int bt[],int n,int count)
{
	int i,res=-1,min=INT_MAX;
	for(i=0;i<n;i++)
	{
		if(at[i]<=count && bt[i]>0 && bt[i]<min)
		{
			min=bt[i];
			res=i;
		}
...
void findCt(int at[],int bt[],int ct[],int n,int ctt[],int c,int pn[])
{
	int i,count=0;
	/*if(count==at[0])
	{
	...
	while(c!=0)
	{
		int min=findMinJob(at,bt,n,count);
		if(min!=-1)
		{
...
int main()
{
	...
	scanf("%d",&n);
	int at[n],bt[n],pn[n],ct[n],ctt[n],tat[n],wt[n],btt[n];
	...
	for(i=0;i<n;i++)
	{
		printf("\n%d\t%d\t%d",pn[i],at[i],bt[i]);
	}
	findCt(at,bt,ct,n,ctt,c,pn);
	printf("\n");
	for(i=0;i<n;i++)
	{
		printf("%d\t",ctt[i]);
	...
	for(i=0;i<n;i++)
	{
		printf("%d\t%d\t%d\t%d\t%d\t%d\n",pn[i],at[i],btt[i],ctt[i],tat[i],wt[i]);
	}
